...
internal class SqsPoller(
            ...
            consumer.config.retries,
            {
              when (it) {
                is AwsServiceException ->
                    consumer.onError(ConsumptionError.AwsError(details = it.awsErrorDetails()))
  ...
  private suspend fun acknowledge(messages: List<Message>, queueUrl: URL) {
    ...
    val request =
        DeleteMessageBatchRequest.builder().queueUrl(queueUrl.toString()).entries(entries).build()
